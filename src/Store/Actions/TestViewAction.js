    ...
    if (showImageIds.length === 0 || showImageIds[0].length === 0) {
        ...
        try {
            ...
            if (imageObjList.every((v) => (v.metaData.imageLaterality === undefined || v.metaData.imageLaterality === ''))) {
                ...
                canvasHeight = Math.floor(canvasHeight / imageRow);
...
export const changeHangingLayout = (type) => (dispatch, getState) => {
    ...
    batch(() => {
        ...
        dispatch({
            type: TEST_VIEW_FULL_IMAGEVIEWER,
            payload: '-1_-1'
        });
    });
};
...
export const setFullImageViewer = (viewerIndex) => (dispatch, getState) => {
    dispatch({
        type: TEST_VIEW_FULL_IMAGEVIEWER,
        payload: viewerIndex
    });
...
export const setFocusImageId = (imageId) => (dispatch, getState) => {
    dispatch({
        type: TEST_VIEW_FOCUS_VIEWER_INDEX,
        payload: imageId
    });
}