...
export default class TestSets extends Component {
    ...
    getItemList() {
        Apis.testSetsList(this.state.currentPage).then((data) => {
            this.setState({
                loading: false,
                items: data.data,
    ...
    deleteItemPermanently() {
        ...
        Apis.testSetsDelete(selectedItem.id).then(resp => {
    ...
    addNewItem() {
        ...
        if (addNewItemDetail.name !== '' && addNewItemDetail.modality_id !== '') {
            this.setState({addNewItemModal: false, loading: true});
            let self = this;
            Apis.testSetsAdd(this.state.addNewItemDetail).then(resp => {
    ...
    updateItem() {
        ...
        if (editItem.name !== '' && editItem.modality_id !== '') {
            ...
            let self = this;
            Apis.testSetsUpdate(editItem.id, editItem).then(resp => {
    ...
    render() {
        ...
        return (
            <div className="item-management">
                <Helmet>
                    <title>Test Sets Management</title>
                    ...
                    <div className="table-responsive">
                        <div className="d-flex justify-content-between py-20 px-10 border-bottom">
                            ...
                            <div>
                                ...
                                <a href="javascript:void(0)" onClick={() => this.opnAddNewItemModal()} color="primary" className="caret btn-sm mr-10">Add New Item <i
                    ...
                    <ModalBody>
                        {editItem === null ?
                            <AddNewForm
                                ...
                                modalitiesList={this.state.modalitiesList}
                            />
                            : <UpdateForm item={editItem} onUpdateItemDetail={this.onUpdateItemDetails.bind(this)} modalitiesList={this.state.modalitiesList}/>
                        }
                    </ModalBody>
                    <ModalFooter>
                        {editItem === null ?
                            ...
                            <div className="clearfix d-flex">
                                <div className="media pull-left">
                                    <div className="media-body">
                                        <p>Name: <span className="fw-bold">{selectedItem.name}</span></p>
                                        <p>Target State: <span className="badge badge-warning">{selectedItem.target_state}</span></p>
                                        <p>Target Country: <span className="fw-bold">{selectedItem.target_country}</span></p>
                                        <p>Archived: <span className="fw-bold">{selectedItem.archived}</span></p>
                                        <p>Modality: <span className="fw-bold">{selectedItem.modalities.name}</span></p>
                                        <p>Difficulty: <span className="fw-bold">{selectedItem.difficulty}</span></p>
                                        <p>Created At: <span className="fw-bold">{moment(selectedItem.created_at).format('MMMM Do YYYY, HH:mm:ss')}</span></p>
                                        <p>Updated At: <span className="fw-bold">{moment(selectedItem.updated_at).format('MMMM Do YYYY, HH:mm:ss')}</span></p>
                                    </div>
                                </div>
                            </div>
                        </div>
